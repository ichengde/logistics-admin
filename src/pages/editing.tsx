...
export default (props) => {
  ...
  const requestLogisticsList = (orderId) => {
    request.post(`/logistics/address/list/${orderId}`).then((response) => {
      let { data } = response;
      data = data.map((p) => {
  ...
  const onFinish = (values) => {
    ...
    const ss = function (response) {
      message.info(
        response.message ?? (path === '/entering' ? '保存成功' : '更新成功'),
      );
  ...
  return (
    <div className={styles.layout}>
      <Card style={{ margin: 20 }}>
        ...
        >
          {isEntering === false && (
            <Form.Item label="id" name="_id" rules={[{ required: isEntering }]}>
                  ...
                  render: (text, record: any) => (
                    <Space size="middle">
                      <Popconfirm
                        ...
                        onConfirm={() => {
                          request
                            .get(`/logistics/address/delete/${record._id}`)
                            .then((response) => {
